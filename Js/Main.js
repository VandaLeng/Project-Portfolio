...
// Cyber grid background animation with network nodes and lines in blue on black
function createCyberGridBackground() {
    ...
    function drawGrid() {
        ...
        if (document.body.classList.contains("light-theme")) {
            ctx.fillStyle = "rgba(255, 255, 255, 0.97)" // Almost solid white for light theme
            ctx.fillRect(0, 0, canvas.width, canvas.height)
        } else {
            ctx.fillStyle = "rgba(0, 0, 0, 0.97)" // Almost solid black for dark theme
            ctx.fillRect(0, 0, canvas.width, canvas.height)
        }
    ...
    if (themeToggle) {
        themeToggle.addEventListener("change", () => {
            // The background color will be updated in the next animation frame
        })
    }

    // Handle skill card hover to create push effect
    document.addEventListener("mouseover", (e) => {
        if (e.target.closest(".skill-card")) {
            ...
            // Find nodes near the card and push them away
            nodes.forEach((node) => {
...
// GSAP for Profile Image Animation with slower vertical movement
function animateProfileImage() {
    if (typeof gsap !== "undefined") {
        ...
        gsap.fromTo(
            profileImage, { y: -20, opacity: 0, scale: 0.9 }, {
                y: 0,
                opacity: 1,
                scale: 1,
                duration: 2,
        ...
        // Slower vertical floating animation
        gsap.to(profileImage, {
            y: 10, // Reduced movement range
            yoyo: true,
            repeat: -1,
            duration: 6, // Slower duration
            ease: "sine.inOut",
        })
...
// Theme Toggle Functionality with fixed light/dark mode
...
function applyLightTheme() {
    ...
    // Update skill card colors for light theme
...
function applyDarkTheme() {
    ...
    // Update skill card colors for dark theme
    document.querySelectorAll(".skill-card").forEach((card) => {
...
// Initialize skills cards with animation and 3D effect
function initializeSkillsCards() {
    ...
    // Add data-aos attributes for AOS animations
